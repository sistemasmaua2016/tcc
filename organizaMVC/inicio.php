...
$d1 = date('Y-'.$mes_num["$mes"].'-1');
$d2 = date('Y-'.$mes_num["$mes"].'-31');
$resu1 = mysql_query("SELECT * FROM `financas` WHERE `usuario_id` = '$id' AND `tipo` = 'despesa' AND `data` BETWEEN ('$d1') AND ('$d2')");
$totalapag = 0;
if ($resu1){
while ($row = mysql_fetch_assoc($resu1)){
$totalapag = $totalapag + $row['valor'];
}
}
$resu2 = mysql_query("SELECT * FROM `financas` WHERE `usuario_id` = '$id' AND `tipo` = 'receita' AND `data` BETWEEN ('$d1') AND ('$d2')");
$totalarec = 0;
if ($resu2){
while ($row = mysql_fetch_assoc($resu2)){
$totalarec = $totalarec + $row['valor'];
}
}
?>
</div>
...
<div id="lista" style="float: right; width: 50%; height: 50%; position: absolute; left: 46%; top: 8%; font-family: 'Bree Serif', serif; font-size: 16px; color: #444;">
  <table width="437" border="0">
    ...
    <tr>
<?php
$data = date('Y-'.$mes_num["$mes"].'-d');
if (date('d') < 10){
	$data2 = date('Y-'.$mes_num["$mes"].'-10');
}
if (date('d') > 10 and date('d') < 20){
	$data2 = date('Y-'.$mes_num["$mes"].'-20');
}
if (date('d') > 20 and date('d') < 30){
	$data2 = date('Y-'.$mes_num["$mes"].'-30');
}
$avencer = mysql_query("SELECT * FROM `financas` WHERE usuario_id='$id' and `categoria`!= 'paga' and `categoria`!= 'recebida' and `categoria`!= 'vencida' and data_venc BETWEEN ('$data') AND ('$data2')");
$contalinhas = mysql_num_rows($avencer);
?>
<?php if($avencer) : ?>
<?php while ($res = mysql_fetch_assoc($avencer)): ?>
      <td width="38" align="left"><?php $d = $res['data_venc']; echo date('d/m/Y', strtotime($d)); ?></td>
      <td width="111" align="right"><?php echo $res['tipo']; ?></td>
      <td width="160" align="right"><?php echo $res['descricao']; ?></td>
      <td width="110" align="right"><?php echo 'R$ '.number_format($res['valor'],2,",","."); ?></td>
    </tr>
    <?php endwhile; ?>
<?php endif; ?>
<td><?php if($contalinhas < 1){ echo 'Nenhum registro encontrado!';} ?></td>
</table>
</div>
...
<table width="0" border="0" cellspacing="0" cellpadding="0" align="center" style="height: 77%;">
  <tr>
    <td><a onclick="document.getElementById('mes').value='Jan'; document.getElementById('fmes').submit()" class="btn btn-primary" style="width: 95%;">Janeiro</a></td>
    <td><a onclick="document.getElementById('mes').value='Feb'; document.getElementById('fmes').submit()" class="btn btn-primary" style="width: 95%;">Fevereiro</a></td>
  ...
  <tr>
    ...
    <td><a onclick="document.getElementById('mes').value='Jun'; document.getElementById('fmes').submit()" class="btn btn-primary" style="width: 95%;">Junho</a></td>
    <td><a onclick="document.getElementById('mes').value='Jul'; document.getElementById('fmes').submit()" class="btn btn-primary" style="width: 95%;">Julho</a></td>
    <td><a onclick="document.getElementById('mes').value='Aug'; document.getElementById('fmes').submit()" class="btn btn-primary" style="width: 95%;">Agosto</a></td>
  </tr>
  <tr>
    <td><a onclick="document.getElementById('mes').value='Sep'; document.getElementById('fmes').submit()" class="btn btn-primary" style="width: 95%;">Setembro</a></td>
    <td><a onclick="document.getElementById('mes').value='Oct'; document.getElementById('fmes').submit()" class="btn btn-primary" style="width: 95%;">Outubro</a></td>
    <td><a onclick="document.getElementById('mes').value='Nov'; document.getElementById('fmes').submit()" class="btn btn-primary" style="width: 95%;">Novembro</a></td>
    <td><a onclick="document.getElementById('mes').value='Dec'; document.getElementById('fmes').submit()" class="btn btn-primary" style="width: 95%;">Dezembro</a></td>
  </tr>
</table>
</div>
</div>
