...
<tbody>
<?php $total = 0; $totresult = mysql_num_rows($result); ?>
<?php if ($result) : ?>
<?php while($row = mysql_fetch_assoc($result)) : ?>
		...
		<td><?php $data = $row['data']; echo date('d/m/Y', strtotime($data)); ?></td>
        <td><?php $datvenc = $row['data_venc']; echo date('d/m/Y', strtotime($datvenc)); ?></td>
		<td class="actions text-right"><a onclick="document.getElementById('idalt').value='<?php echo $row['id']; ?>'; document.getElementById('tituloalt').value='<?php echo $row['titulo']; ?>'; document.getElementById('valoralt').value='<?php echo 'R$ '.number_format($row['valor'],2, ',', '.');?>'; document.getElementById('descricaoalt').value='<?php echo $row['descricao']; ?>'; document.getElementById('categoriaalt').value='<?php echo $row['categoria']; ?>'; document.getElementById('datavencalt').value='<?php $datvenc = $row['data_venc']; echo date('d/m/Y', strtotime($datvenc)); ?>'; modificamodal()" href="#ModalEdit" class="btn btn-sm btn-warning"><i class="fa fa-pencil"></i>Editar</a>
        <a onclick="document.getElementById('idd').value='<?php echo $row['id']; ?>'; location.href='#ModalDel'; document.getElementById('posta').style.visibility='visible'; document.getElementById('cancela').style.visibility='visible'" class="btn btn-sm btn-danger">Excluir</a>
		</td>
	</tr>
<?php endwhile; ?>
<?php endif; ?>
...
?>
<div id="cancela" style="width:27%; height:32%; visibility: hidden; position:absolute; top: 50%; left: 28%" class="btn btn-danger" onclick="location.href='#close'; atualizaIframep()">Cancelar</div>
<div id="fecha" style="width:20%; height:32%; visibility: visible; position:absolute; top: 50%; left: 7%" class="btn btn-primary" onclick="location.href='#close'; atualizaIframep()">OK</div>
...
<div id="okat" style="width:20%; height:23%; position:absolute; top: 60%; left: 6.5%" class="btn btn-primary" onclick="location.href='#close'; atualizaIframep()">OK</div><div style="float:right">
